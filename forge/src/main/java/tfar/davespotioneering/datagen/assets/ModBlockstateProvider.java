...
public class ModBlockstateProvider extends BlockStateProvider {
    ...
    protected void brewingStand() {
        getMultipartBuilder(ModBlocks.COMPOUND_BREWING_STAND)
                ...
                .part().modelFile(models().getExistingFile(modLoc("block/advanced_brewing_stand_bottle0"))).addModel().condition(CAdvancedBrewingStandBlock.HAS_BOTTLE[0], true).end()
                .part().modelFile(models().getExistingFile(modLoc("block/advanced_brewing_stand_bottle1"))).addModel().condition(CAdvancedBrewingStandBlock.HAS_BOTTLE[1], true).end()
                .part().modelFile(models().getExistingFile(modLoc("block/advanced_brewing_stand_bottle2"))).addModel().condition(CAdvancedBrewingStandBlock.HAS_BOTTLE[2], true).end()
                .part().modelFile(models().getExistingFile(modLoc("block/advanced_brewing_stand_empty0"))).addModel().condition(CAdvancedBrewingStandBlock.HAS_BOTTLE[0], false).end()
                .part().modelFile(models().getExistingFile(modLoc("block/advanced_brewing_stand_empty1"))).addModel().condition(CAdvancedBrewingStandBlock.HAS_BOTTLE[1], false).end()
                .part().modelFile(models().getExistingFile(modLoc("block/advanced_brewing_stand_empty2"))).addModel().condition(CAdvancedBrewingStandBlock.HAS_BOTTLE[2], false).end()
        ;
    }
}
